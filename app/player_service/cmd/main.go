...
import (
	...
	"one_ser.com/app/player_service/sys_init/redis"
	"one_ser.com/config"
	"one_ser.com/etcd"
	"one_ser.com/logger"
...
func main() {
	...
	addr:=fmt.Sprintf("%s:%d",config.Conf.IP,config.Conf.Services["player"].RpcPort)
	grpcAddress := addr
	...
	// 绑定service
	player_pb.RegisterUserServiceServer(server, &service.UserSrv{})
